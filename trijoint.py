    ...
    def __init__(self):
        ...
        if opts.preModel == 'resNet50':
            ...
            self.visual_embedding = nn.Sequential(
                nn.Linear(opts.imfeatDim + opts.imgCLSVecDim, opts.embDim),
                nn.BatchNorm1d(opts.embDim),
                nn.LeakyReLU(0.02, inplace=True),
            )

            self.tfidf_embedding = nn.Sequential(
                nn.Linear(opts.ingrW2VDim, opts.TFIDFDim),
                nn.LeakyReLU(0.02, inplace=True),
                )

            self.recipe_embedding = nn.Sequential(
                nn.Linear(opts.TFIDFDim + opts.srnnDim, opts.embDim),
                nn.BatchNorm1d(opts.embDim),
                nn.LeakyReLU(0.02, inplace=True),
            )
